...
class Transactions:
    ...
    def read_txs(self):
        ...
        #print("Version of User Transaction: ", version_of_user_tx)
        ...
        if input_txs_or_witness_marker == 0:
            ...
            #print("Input Transactions: ", input_txs_or_witness_marker)
            ...
            for each_input_tx in range(input_txs_or_witness_marker):
                ...
                if input_txs_or_witness_marker == 1 and prev_tx_id == "0x" and prev_tx_out_index == -1:
                    # tx_in=({"address":"MINING_REWARD","previous_tx_id": prev_tx_id,"previous_tx_index" : prev_tx_out_index})
                    tx_in=({"btc_address":"MINING_REWARD","previous_tx_id": prev_tx_id,"previous_tx_index" : prev_tx_out_index})
                else:
                    #tx_in.append(["NO_ADDRESS", prev_tx_id, prev_tx_out_index])
            ...
            #print("\nOutput Transactions: ", output_txs)
        ...
        # if lock time is recorded below 500 million, 
        # it is interpreted as the specific block height
        # if lock_time < 500000000:
        #    print("Lock Time is block height: ", lock_time,"\n\n") 
            
        # is set larger than 500 million, it means that the timelock is recored as unix timestamp, 
        # and this transaction can only be valid after the specified time.
        # else:
        #    print("Lock Time is timestamp\n\n")
        ...
        #tx_data["tx_id"] = tx_id
        #tx_data["timestamp"] = self.timestamp

        # also create a tx_id.pickle file with the out data
        #tx_id_file = open(self.tx_id_path+tx_id+".pickle", "wb")
        #cPickle.dump(tx_data["tx_out"], tx_id_file)
        #tx_id_file.close()

        # cPickle.dump(tx_data, self.pickle_file) #indent = 2
        # print("......................................................")
        ...
        if witness_flag != 0:
            ...
            for s in range(input_txs):
                ...
                for st in range(stack_items):
                    ...
                    #print("Witness Item: ",witness_item)
