...
class Main:
    ...
    # exp_path = /../Desktop/Transactions_info/
    # exp_path = Paths.get_export_path(input_path)
    c_dir = Paths.get_export_path(input_path)
    for d in os.listdir(c_dir): shutil.rmtree(c_dir+d)
    #exp_path = Paths.check_export_path(input_path, exp_path)

    # check if directory exists, and overwrite it or not  
    # c_dir = Paths.export_directory_exists(exp_path)
    ...
    # tx_in_csv, tx_out_csv, timestamp_info_csv = Paths.create_files(tx_inputs_path, tx_outputs_path, timestamp_info_path)
    
    # all in one
    # tx_in_csv, tx_out_csv, timestamp_info_csv = Paths.create_files(c_dir)
    ...
    # if  input_path != c_dir.rsplit('/',2)[0]+"/":
