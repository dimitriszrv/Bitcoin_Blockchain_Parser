...
class Paths:
    ...
    def export_directory_exists(c_dir):
        c_dir += "Transactions_info/"
        while True:
            if not os.path.exists(c_dir):
                try:
                    ...
                    print("> Error while creating directory '%s'" % c_dir)
            else:
                print("\n> Export directory '%s' already exists" %c_dir)
                exp_choice = input("> Do you want to overwrite it? [y/n]\t> ")
                while True:
                    if exp_choice == "y" or exp_choice == "n":
                        break
                    else:
                        print("\n> Wrong choice, choice must be 'y' or 'n'")
                        print("> If you want to quit press 'q'")
                        exp_choice = (input("> Do you want to overwrite it? [y/n]\t> ")).lower()
                        if exp_choice == "q": 
                            print("> Exiting...")
                            print("---------------------------------------------------------------------------------------------\n")
                            exit(-1)
                if exp_choice == "y": 
                   ...
                   c_dir = Paths.get_export_path() 

        #i,o,t,f_d = Paths.create_dirs(c_dir)
        return c_dir
    ...
    def create_dirs(c_dir):
        ...
        ## get path to create folder with final data 
        ## stores final information about transactions, tx_id | sender | receiver | value | timestamp
        #get_path = c_dir.rsplit('/',2)[0]
        #transactions_path = get_path+"/Bitcoin_Transactions"
        #if os.path.exists(transactions_path): shutil.rmtree(transactions_path)
        #os.mkdir(transactions_path)

        return transaction_inputs, transactions_outputs, timestamp_info#, transactions_path

    #def create_files(c_dir):
    #    # create csv file with transaction inputs
    #    tx_in_csv = open(c_dir+"transaction_inputs.csv","w")
    #    tx_in_csv.write("tx_id\tbtc_address\tprevious_tx_id\tprevious_tx_index\n")
    #            
    #    # create csv file with transaction outputs
    #    tx_out_csv = open(c_dir+"transaction_outputs.csv","w")
    #    tx_out_csv.write("tx_id\tbtc_address\tbtc_value\ttx_index\n")    
#
    #    # create csv file with timestamp information
    #    time_info = open(c_dir+"timestamp_info.csv","w")
